...
minRowHeight | number | undefined  | Optional. Gallery will try to make the elements use at least the supplied height. Please note that if the element is using the entire row for itself it might bypass the restriction.
